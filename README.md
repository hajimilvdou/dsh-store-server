...
**① GitHub 自动收录插件库**
- 按 `topic:dsh-plugin` 搜索 GitHub 仓库，自动提取 README 简介、分类、安装 spec（npm 包名 / `github:owner/repo` 自动回落）；
- 星数快照 + 7 天趋势榜（`stars_7d`），趋势/热门/最新多维排序；
- 多 token 池轮换抓取，页间限速防限流，进度与结果全程可视化。

**② 联邦互联（多服务器互连）**
- 服务器间经握手 + 密码互换建立关系，可选择性共享：插件 / Agent 预设 / 组合 / 用户；
- 单向解约即断开，对方收到通知并保留已同步数据；心跳保活 + 增量同步。

**③ 完整的多方数据模型**
- **插件**（GitHub 同步）、**Agent 预设**（git 仓库内化安装）、**组合**（combos：用户自编插件组合，含安装模式/订阅/自动发布/启动复核）、**用户**（GitHub OAuth 登录/注册/封禁/注销）；
- 组合订阅数、下载计数（匿名凭证 + 1h 去重）、点赞风控（注册即赞/同 IP 集中赞/高频切换 → 待确认队列）；
- 公告（SSE 实时推送）、客户端版本推送（面板可配安装地址）。

**④ 管理端一体化（/admin）**
- 仪表盘：同步进度 / 联邦关系 / 风险队列 / 安全监控 / 磁盘余量；
- **配置中心**：搜索 token、OAuth、JWT、管理员密码、联邦密码、访问口令、注册开关等**每项独立保存、保存即热更新**，密钥不回显（仅"已配置/未配置"）；
- 风控队列复核、系统更新面板（**一键检测 + 一键在线更新**，容器部署走面板热更新、失败自动回滚）+ 客户端插件版本推送；
- 首次使用强制设置管理员密码（≥8 位，先到先得），无任何硬编码后门口令。

**⑤ 安全设计**
- 读写限流分档守卫（429 + 告警 Webhook 热更新）；时钟漂移自检（>500ms 告警，>5s 拒签凭证）；
- 所有密钥只来自环境变量或配置表，绝不入库/写死；生产未配置管理员口令时管理端整体 503；
- 匿名写接口强制 `X-Anon-Token`；联邦接口独立 `X-Federation-Secret`。

**⑥ 部署与运维省心**
- **零门槛一键远程部署**：服务器只要装了 docker，一条 `curl | bash` 从零拉起（拉镜像 → 建库 → 迁移 → 起服务 → 自检）；
- GitHub Actions 自动构建 GHCR 镜像（无需任何 Secrets）；compose / 纯 docker 命令双通道；
- 一键更新带数据库迁移 + 自检 + 自动回滚；迁移只加不减（expand-contract），回滚无需还原数据库；
- 非必要数据默认保留 2 天自动清理（可配）。
